...
const ListeFamangina = ({ title, filterValues0, getAllMpiangona,autreTitle,changeDeclarer }) => {
    ...
    const titleTable = [
        { title: "Numero Fiche", data: "numfichesuivie", typeData: 'input' },
        { title: "Daty Namangina", data: "datesuivie", typeData: 'date' },
        { title: "Namangy", data: "namangy", typeData: 'input' },
        { title: "Description", data: "descriptionsuivie", typeData: 'input' },
        { title: "Nom/Prenom Dekonina", data: "nomcompletmpiangona", typeData: 'input' },
        {
            title: "LAHY/ VAVY Dekonina", data: "codegenrempiangona", typeData: 'select', getOptions: () => {
                        ...
                        <thead>
                            <tr>
                                {titleTable.map(
                                    (column, index) =>
                                        (!column.isExtra || showExtraColumns) && (
                                            <th key={index} style={{ backgroundColor: "#c85d09", color: "#fff", border: "1px solid #fff", textAlign: 'center' }}>
                                                {column.title}
                                            </th>
                                        )
                                )}
